...
def main(
    ...
    data_available = search_files(
        dataset=dataset, start_date=start_date, end_date=end_date, version=dataset_version
    )
    ...
    for granule in data_available:
        ...
        if os.path.exists(outfile):
            print(f"File {outfile} already exists, skipping download & gridding.")
            continue
        ...
        if filename.endswith(".nc4"):
            da, datestr = read_nc(file=file, var_name=varname, grid_bounds=grid_bounds)
        elif filename.endswith(".h5") or filename.endswith(".he5"):
            da, datestr = read_h5(file=file, var_name=varname, grid_bounds=grid_bounds)

        # 3) interpolate the variable to the grid
        df = interp_to_grid(da=da, grid=grid, var_name=varname, datestr=datestr)

        # 4) save the interpolated data
        data_out = df[["grid_id", "date", varname]].rename(columns={varname: var_label})
...
def get_grid():
    ...
    shapefile = "grid_india_10km_shapefiles/grid_india_10km"
...
def read_nc(file, var_name, grid_bounds):
    ...
    with xr.open_dataset(file) as ds:
        ...
        ds = ds.sel(
            lon=slice(grid_bounds[0] - 1, grid_bounds[2] + 1),
        ...
        if len(ds.dims) != 2:
            raise ValueError(f"Data is not 2D for projection: dimensions are {ds.dims}")
...
def read_h5(file, var_name, grid_bounds):
    ...
    if var_name in [
        ...
        ds = xr.open_dataset(file, group="HDFEOS/GRIDS/ColumnAmountNO2/Data Fields")
        ...
        resolution = eval(f["HDFEOS/GRIDS/ColumnAmountNO2"].attrs["GridSpacing"])
        ...
        assert lat_len == len(lat), f"lat length {lat_len} does not match grid length {len(lat)}"
        assert lon_len == len(lon), f"lon length {lon_len} does not match grid length {len(lon)}"
        ...
        raise ValueError(f"Unknown variable {var_name} to read from file {file}")

    return da, datestr


def define_coordinates(lat_bounds: List[float], lon_bounds: List[float], resolution: List[float]):
