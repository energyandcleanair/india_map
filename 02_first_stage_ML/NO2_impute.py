...
def run_outer_cv(df, features, target, group_col, best_params, output_dir):
    ...
    for fold, (trn_idx, val_idx) in enumerate(outer_cv):
        ...
        model_xgb = XGBRegressor(**best_params, n_jobs=int(os.getenv("SLURM_CPUS_PER_TASK", 4)), tree_method='gpu_hist')
...
def main():
    ...
    if os.path.exists(fe_file):
        ...
        print(f"Feature-engineered data loaded from {fe_file} with shape: {df_fe.shape}")
    else:
        print(f"Feature-engineered file {fe_file} not found. Please run the feature engineering script first.")
        sys.exit(1)
    
    # Step 2: Load the sampled training data for the NO₂ model
    ml_input_file = os.path.join(path_to_data, "ML_full_model", "NO2_ml_df_sampled.csv")
