...
def load_and_sample_df():
    ...
    output_sampled = os.path.join(
        path_to_data, "ML_full_model", "aod_ml_df_sampled.csv")
...
def run_outer_cv(df, features, target, group_col, best_params_XGB, output_dir):
    ...
    for fold, (trn_idx, val_idx) in enumerate(outer_cv):
        ...
        model_xgb = XGBRegressor(
            **best_params_XGB, n_jobs=int(os.getenv("SLURM_CPUS_PER_TASK", 4)), tree_method='gpu_hist')
...
def main():
    ...
    missing_file = os.path.join(
        path_to_data, "ML_full_model", "aod_missing_to_be_imputed.csv")
