...
def test__validate_result_schema__invalid_row_count__raises_error():
    ...
    with pytest.raises(ValueError, match="Expected 100 rows in path/to/result, but found 50 rows."):
        validator.validate_result_schema(expected_result)
...
def test__validate_result_schema__missing_column__raises_error():
    ...
    with pytest.raises(
        ValueError, match="Expected 'date' column in path/to/result, but it is missing."
    ):
        validator.validate_result_schema(expected_result)
