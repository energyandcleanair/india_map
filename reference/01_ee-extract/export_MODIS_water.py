# export_MODIS_water.py
import ee
from gee_utils import process_yearly, grid, get_export_properties

OUTPUT_DRIVE = "MODIS_water_FIN"
start_date = "2005-01-01"
end_date = "2023-01-01"

# Define your MODIS water product.
# (For example, you may choose a land cover type that represents water.)
# This example uses a mask on LC_Type1; adjust as needed.
maiac_old = ee.ImageCollection("MODIS/061/MCD12Q1").select("LC_Type1") \
    .map(lambda img: img.updateMask(img.select("LC_Type1").eq(17))) \
    .map(lambda img: img.addBands(
        img.select("LC_Type1")
           .where(img.select("LC_Type1").eq(17), 1)
           .where(img.select("LC_Type1").neq(17), 0)
...
output_name = "MODIS_water_" + start_date + "_to_" + end_date
