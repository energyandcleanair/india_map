...
# collect input features for these grid polygons
GRID_ASSET = "path_to_your_asset_in_gogole_earth_engine/grid_india_10km"
...
def create_image(start_date, length_date, collection):
    ...
    for i in range(length_date):
        im_date = ee.Date.fromYMD(
            start_dt.year, start_dt.month, start_dt.day).advance(i, 'day')
        im = collection.filterDate(im_date, im_date.advance(
            1, 'day')).reduce(ee.Reducer.mean())
...
def process_yearly(start_date, end_date, collection, reducer=ee.Reducer.mean(), scale=10000, crs='EPSG:7755'):
    ...
    for i in range(years):
        im_date = ee.Date.fromYMD(
            start_date.year + i, start_date.month, start_date.day)
        im = collection.filterDate(im_date, im_date.advance(
            1, 'year')).reduce(ee.Reducer.mean())
        im = ee.Image(im).set("start_date", im_date.format("yMMdd"))
        images_list = images_list.add(im)
